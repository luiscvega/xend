...
test "formula with weight with tax" do
  rate = Xend::Formula.calculate(Xend::MetroManila_RATES, weight: 1)
...
test "formula via dimensions" do
  weight = Xend::Formula.calculate_weight(length: 12, width: 12, height: 10)
  assert_equal 0.42, weight
end

test "formula with heavy shipment" do
  rate = Xend::Formula.calculate(Xend::MetroManila_RATES, weight: 4)
  api_rate = Xend::Rate.calculate(type: Xend::MetroManila,
                                  weight: 4, length: 10, width: 10, height: 10)
  assert_equal api_rate.to_f, rate
end

# test "forumla with complete information (Weight & Dimensions)" do
...
# end
# 
# test "forumla with complete information (Weight & Dimensions) but volume weight is followed" do
#   params = { weight: 1, length: 20, width: 20, height: 20 }
#   rate = Xend::Formula.calculate_complete(Xend::MetroManila_RATES, params)
#   api_rate = Xend::Rate.calculate({ type: Xend::MetroManila }.merge!(params))
# 
#   assert_equal api_rate.to_f, rate
...
test "calculate small Rizal" do
  rate = Xend::Rate.calculate(type: Xend::Rizal,
                              weight: 0.5, length: 0, width: 0, height: 0)
  assert_equal "49.280000", rate
end
