...
test "formula with weight with tax" do
  rate = Xend::Formula.calculate(Xend::METRO_MANILA_RATES, 1)
  api_rate = Xend::Rate.calculate(type: Xend::METRO_MANILA,
                                  weight: 1, length: 10, width: 10, height: 10)
  assert_equal api_rate.to_f, rate
end

test "formula via dimensions" do
  weight = Xend::Formula.volume_weight(12, 12, 10)
  assert_equal 0.42, weight
end

test "formula with heavy shipment" do
  rate = Xend::Formula.calculate(Xend::METRO_MANILA_RATES, 4)
  api_rate = Xend::Rate.calculate(type: Xend::METRO_MANILA,
...
test "formula with complete information (Weight & Dimensions)" do
  ...
  rate = Xend::Formula.calculate_complete(Xend::METRO_MANILA_RATES, params)
  api_rate = Xend::Rate.calculate({ type: Xend::METRO_MANILA }.merge!(params))
...
test "calculate small Metro Manila" do
  rate = Xend::Rate.calculate(type: Xend::METRO_MANILA,
                              weight: 1, length: 10, width: 10, height: 10)
...
# ShipmentService
test "create" do
  info = { type: Xend::METRO_MANILA, weight: 1, length: 10, width: 10, height: 10,
